...
int main( int argc, char *argv[ ] )
{
   // . . .
    ...
    float quartTileArea = fullTileArea/4.0;
    
    //printf("Full is: %8.8lf\nHalf is: %8.8lf \nQuarter is: %8.8lf\n\n", fullTileArea, halfTileArea, quartTileArea);
    
    double time0 = omp_get_wtime();
    
    // sum up the weighted heights into the variable "volume"
    // using an OpenMP for loop and a reduction:
    #pragma omp parallel for default(none) shared(fullTileArea,halfTileArea,quartTileArea) reduction(+:_volume)
    for( int i = 0; i < NUMNODES*NUMNODES; i++ )
    {
        ...
        //. . .
        if(((iu == 0) || (iu == (NUMNODES-1)) && ((iv == 0) || (iv == (NUMNODES-1))))){// if corner
            _volume += quartTileArea*h;
        }
        else if(iu == 0 || iv == 0 || iu == (NUMNODES-1) || iv == (NUMNODES-1)){// if edge
            _volume += halfTileArea*h;
        }
        else{//if center
            _volume += fullTileArea*h;
        }
    }
    ...
    double megaHeightsPerSecond = ((double)NUMNODES*(double)NUMNODES) / ( time1 - time0 ) / 1000000.;
            ...
            printf("NUMT was: %d\n", NUMT);
            printf("NUMNODES was: %d\n", NUMNODES);
            printf("Volume was found to be: %8.8lf\n", _volume);
            printf("Performance in megaHeightsPerSecond was: %f\n", megaHeightsPerSecond);
   // ?????
}
